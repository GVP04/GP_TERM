...
BOOL CALLBACK SvcEnumComboChilds(HWND hwnd, LPARAM lParam)
{
	HWND *ArrHWND=(HWND *)lParam;
	BOOL ret=TRUE;
	if (ArrHWND) 
	{
		char cName[200];
		*cName=0;
		...
		if (*cName=='E' || *cName=='e') ArrHWND[0]=hwnd;
		if (*cName=='L' || *cName=='l') ArrHWND[1]=hwnd;
		if (ArrHWND[0] && ArrHWND[1]) ret=FALSE;
	}
	return ret;
}
...
#else
			sprintf(oStr,"Minimum operating systems - Windows XP");
#endif		
		}
		break;
	...
	case 67: /*DoListMethod*/
		{
...
#ifdef W_XP
			{
				...
				cbi.cbSize=sizeof(cbi);
				SendMessage(CB_GETCOMBOBOXINFO,0,(LPARAM)&cbi);
				hwndItem=cbi.hwndItem;
				hwndList=cbi.hwndList;
			}
#else
			{
				HWND ArrHWND[]={NULL,NULL,NULL,NULL,NULL,};
				EnumChildWindows(m_hWnd,  SvcEnumComboChilds, (LPARAM)(ArrHWND));
				if (!ArrHWND[1]) ArrHWND[1]=ArrHWND[0]+2;
			}
