	...
	case WM_DRAWITEM:
		if (lParam)
		{
			...
			if (lpds->CtlType==ODT_MENU)
			{
				...
				if (gpu)
				{
					GPC.m_View->m_ODContextsArray->PaintByDRAWITEMSTRUCT(gpu, wParam, lpds, m_GPMenu);
					return TRUE;
				}
			}
		}
	...
	case WM_MEASUREITEM:
		if (lParam)
		{
			...
			if (lpds->CtlType==ODT_MENU)
			{
				...
				if (gpu)
				{
					return GPC.m_View->m_ODContextsArray->MeasureItem(gpu, wParam, lpds, m_GPMenu);
				}
			}
		}
		break;
...
BOOL CDlgContextMenu::InitAndReload()
{
	if (m_Menu) delete m_Menu;
	m_Menu=NULL;
	if (m_GPMenu) delete m_GPMenu;
	...
	m_GPMenu=new GPMenu(m_hWnd,1);
	m_Menu=new CGPContextMenu(m_GPMenu);
	m_Menu->Load(GPC.contmenuFileName);

	m_Menu->SetODCForMenu(GPC.m_View->m_ODContextsArray->pTerminalUserMenuContext, m_Menu->m_Menu);
		...
		case CMCMD_APPLY:
			{
				...
				MenuFromTree(TVI_ROOT,m_Menu->m_Menu);
				m_Menu->Save(CDlgContextMenu::LastContextMenuFile);
				GPC.m_pContextMenu->Load(CDlgContextMenu::LastContextMenuFile);
			}
			break;
		case CMCMD_CURRENT:
			GPC.m_pContextMenu->Save(CDlgContextMenu::LastContextMenuFile);
			InitAndReload();
			m_Menu->FillTree(m_Tree.m_hWnd,TRUE);
			break;
			...
			case TBN_DROPDOWN:
				{
					...
					if (nItem>=0)
					{
						...
						::TrackPopupMenu(m_Menu->m_Menu,0,re.right-15,re.bottom,0,m_hWnd,NULL);
					}
				}
				break;
			}
...
void CDlgContextMenu::OnSelchangedTree(NMHDR* pNMHDR, LRESULT* pResult)
{
	...
	if (sel)
	{
		PGPMENUINFO mi=(PGPMENUINFO)m_Tree.GetItemData(sel);
		if (mi)
		{
			m_dwUpdateFlag=0;
...
HTREEITEM CDlgContextMenu::CopyTree(HTREEITEM hDestParent, HTREEITEM hSrc, HTREEITEM insAfter)
{
	...
	if (hNewItem)
	{
		...
		PGPMENUINFO it=new GPMENUINFO;
		CGPContextMenu::ClearItem(it);
		PGPMENUINFO itOld=(PGPMENUINFO)m_Tree.GetItemData(hSrc);
...
void CDlgContextMenu::CopyChildren(HTREEITEM hDestParent, HTREEITEM hSrc)
{
	...
	if (hItem != NULL)
		do
		{
			...
			if (hNewItem)
			{
				PGPMENUINFO itOld=(PGPMENUINFO)m_Tree.GetItemData(hItem);
				PGPMENUINFO it=new GPMENUINFO;
				CGPContextMenu::ClearItem(it);
				if (itOld)	*it=*itOld;
				m_Tree.SetItemData(hNewItem,(DWORD)it);
...
void CDlgContextMenu::OnDeleteitemTree(NMHDR* pNMHDR, LRESULT* pResult)
{
	...
	if (pNMTreeView)
	{
		...
		{
			PGPMENUINFO cmi=(PGPMENUINFO)m_Tree.GetItemData(pNMTreeView->itemOld.hItem);
			if (cmi) delete cmi;
		}
	}
	*pResult = 0;
...
UINT CDlgContextMenu::MenuFromTree(HTREEITEM iItem, HMENU iHMENU)
{
	if (iItem)
	{
		...
		else
		{
			...
			GPMENUINFO MUDst;
			PGPMENUINFO cmisrc=(PGPMENUINFO)m_Tree.GetItemData(iItem);
			MENUITEMINFO mi={0};
		...
		m_Menu->SetODCForMenu(GPC.m_View->m_ODContextsArray->pTerminalUserMenuContext, iHMENU);
	}
