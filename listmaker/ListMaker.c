...
cc ListMaker.cpp -o ListMaker2 -funsigned-char -lc
chmod 777 ListMaker
...
//	printf("DeleteListC %s\n",iName);
	...
	*cListArray[nLists]=0;
	ListPointers[nLists]=new char * [1];
...
long AddList(long idestIndex, long isrcIndex)
{
	...
	if (idestIndex<nLists)
	{
		if (isrcIndex<nLists)
		{
			long newLen=unListsLength[idestIndex]+unListsLength[isrcIndex]+1;
				...
				while(c2<nElem2)
				{
					...
					if (*tmppos2[c2]!=0)
					{
						while(*pos1!=0)
						{
							...
							while((*pos1==*pos2 || *pos2=='^') && *pos1!=0 && *pos2!=']')
								{pos1++;pos2++;}
