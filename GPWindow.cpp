...
void GPWindow::CharOut(unsigned short ch)
{
	...
	case 7:	
		...
		{
			if (PlaySound("ding.wav", NULL, SND_ASYNC)==FALSE)	MessageBeep( 0xFFFFFFFF);   
		}
		else GPC.m_View->m_uLockBeep|=0x6;
		break;
	case 8:
		CursorMoveLeft();
		CharOut(ChrSpace);
...
GPUnit * GPWindow::GetUnitByName(const char *iccName)
{
	GPUnit *ret=NULL;
	if (iccName && WndUnit)
	{
		...
		if (*iccName)
		{
			if (*iccName>='0' && *iccName<='9')	i=atoi(iccName);
			else								i=(*WndUnit)->GetObjectIdByName(iccName);

			if (i>=0 && i<(*WndUnit)->m_NGPUNIT) ret=WndUnit[i];
		}
		else ret=*WndUnit;
	}
