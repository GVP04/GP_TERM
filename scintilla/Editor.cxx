...
//gg
cScPoint Editor::LocationFromPosition(int pos) {
	cScPoint pt;
	RefreshStyleData();
...
//gg
	cScPoint pt = LocationFromPosition(pos);
	return pt.x - vs.fixedColumnWidth + xOffset;
}

//gg
int Editor::LineFromLocation(cScPoint pt) {
...
//gg
int Editor::PositionFromLocation(cScPoint pt) {
	RefreshStyleData();
...
//gg
int Editor::PositionFromLocationClose(cScPoint pt) {
	RefreshStyleData();
...
//gg
	cScPoint pt = LocationFromPosition(currentPos);
	lastXChosen = pt.x;
}
...
//gg
		                   cScPoint(lastXChosen, rcClient.top)),
		               noSel, ensureVisible);
	} else if ((pt.y + vs.lineHeight - 1) > rcClient.bottom) {
		int yOfLastLineFullyDisplayed = rcClient.top + (LinesOnScreen() - 1) * vs.lineHeight;
		MovePositionTo(PositionFromLocation(
//gg
		                   cScPoint(lastXChosen, rcClient.top + yOfLastLineFullyDisplayed)),
		               noSel, ensureVisible);
	}
}
...
//gg
	cScPoint pt = LocationFromPosition(posCaret);
	cScPoint ptBottomCaret = pt;
...
void Editor::PaintSelMargin(Surface *surfWindow, PRectangle &rc) {
	...
	if (bufferedDraw) {
//gg
		surfWindow->Copy(rcMargin, cScPoint(), *pixmapSelMargin);
	}
}
...
//gg
		cScFont &ctrlCharsFont = vstyle.styles[STYLE_CONTROLCHAR].font;
...
//gg to //gg
	if (strchr(ll->chars,2))
	{
		ll->widthLine = width;
		// take into account the space for end wrap mark
		if (wrapVisualFlags & SC_WRAPVISUALFLAG_END) width -= vstyle.aveCharWidth;
		ll->lines = 0;
		...
		{
			if ((ll->positions[p + 1] - startOffset) >= width || (p && ll->chars[p-1]==2)) 
			{
				if (p && ll->chars[p-1]==2) lastGoodBreak=p;
				else
				if (lastGoodBreak == lastLineStart)
				{
					if (p > 0)	lastGoodBreak = pdoc->MovePositionOutsideChar(p + posLineStart, -1)- posLineStart;
					if (lastGoodBreak == lastLineStart) lastGoodBreak = pdoc->MovePositionOutsideChar(lastGoodBreak + posLineStart + 1, 1)-posLineStart;
				}
			...
			if (p > 0)
			{
				...
				{
					if (IsSpaceOrTab(ll->chars[p - 1]) && !IsSpaceOrTab(ll->chars[p]))	lastGoodBreak = p;
				}
			}
			p++;
		}
...
//gg
	cScPoint from(0, ((lineVisible & 1) && (lineHeight & 1)) ? 1 : 0);
...
void DrawTextBlob(Surface *surface, ViewStyle &vsDraw, PRectangle rcSegment,
	...
	cScFont &ctrlCharsFont = vsDraw.styles[STYLE_CONTROLCHAR].font;
...
//gg
				cScFont &textFont = vsDraw.styles[styleMain].font;
				//hotspot foreground
	...
	if (rcArea.right > vs.fixedColumnWidth) {
		...
		while (visibleLine < cs.LinesDisplayed() && yposScreen < rcArea.bottom) {
			...
			if (ll) {
				...
				if (bufferedDraw) {
//gg
					cScPoint from(vs.fixedColumnWidth, 0);
	...
	for (int i = 0; i < len; i++) {
		if (IsEOLChar(ptr[i])) {
			...
			if ((XFromPosition(currentPos) < xInsert) && (i + 1 < len))	{
				//gg
				for (int i333 = 0; i333 < xInsert - XFromPosition(currentPos); i333++) {
...
//gg
void Editor::NotifyDoubleClick(cScPoint pt, bool shift, bool ctrl, bool alt) {
	SCNotification scn = {0};
...
//gg
bool Editor::NotifyMarginClick(cScPoint pt, bool shift, bool ctrl, bool alt) {
	int marginClicked = -1;
	int x = 0;
	for (int margin = 0; margin < ViewStyle::margins; margin++) {
...
//gg
void Editor::NotifyDwelling(cScPoint pt, bool state) {
	SCNotification scn = {0};
...
//gg
	//Platform::DebugPrintf("** Save cScPoint %s\n", atSavePoint ? "On" : "Off");
...
void Editor::PageMove(int direction, selTypes sel, bool stuttered) {
	...
	int bottomStutterLine =
	    pdoc->LineFromPosition(PositionFromLocation(
	                cScPoint(lastXChosen, direction * vs.lineHeight * LinesToScroll())))
	    - caretYSlop - 1;

	if (stuttered && (direction < 0 && currentLine > topStutterLine)) {
		topLineNew = topLine;
//gg
		newPos = PositionFromLocation(cScPoint(lastXChosen, vs.lineHeight * caretYSlop));

	} else if (stuttered && (direction > 0 && currentLine < bottomStutterLine)) {
		topLineNew = topLine;
//gg
		newPos = PositionFromLocation(cScPoint(lastXChosen, vs.lineHeight * (LinesToScroll() - caretYSlop)));
...
//gg
	                 cScPoint(lastXChosen, pt.y + direction * (vs.lineHeight * LinesToScroll())));
	}
...
//gg
	                 cScPoint(lastXChosen, pt.y + direction * vs.lineHeight));
	if (direction < 0) {
...
//gg
		cScPoint ptNew = LocationFromPosition(posNew);
...
//gg
static bool Close(cScPoint pt1, cScPoint pt2) {
...
bool Editor::DragThreshold(cScPoint ptStart, cScPoint ptNow) {
...
bool Editor::PointInSelection(cScPoint pt) {
...
//gg
			cScPoint locStart = LocationFromPosition(pos);
			if (pt.x < locStart.x) {
				return false;
			}
		}
...
//gg
			cScPoint locEnd = LocationFromPosition(pos);
			if (pt.x > locEnd.x) {
				return false;
			}
		}
...
//gg
bool Editor::PointInSelMargin(cScPoint pt) {
	// Really means: "cScPoint in a margin"
...
//gg
void Editor::ButtonDown(cScPoint pt, unsigned int curTime, bool shift, bool ctrl, bool alt) {
...
//gg
bool Editor::PointIsHotspot(cScPoint pt) {
...
//gg
void Editor::SetHotSpotRange(cScPoint *pt) {
	if (pt) {
		int pos = PositionFromLocation(*pt);
...
//gg
void Editor::ButtonMove(cScPoint pt) {
...
//gg
void Editor::ButtonUp(cScPoint pt, unsigned int curTime, bool ctrl) {
...
//gg
			cScPoint pt = LocationFromPosition(lParam);
			return pt.x;
		}
...
//gg
		return PositionFromLocation(cScPoint(wParam, lParam));

	case SCI_POSITIONFROMPOINTCLOSE:
//gg
		return PositionFromLocationClose(cScPoint(wParam, lParam));
	...
	case SCI_MARKERDEFINE:
		...
		break;
//gg
	case SCI_MARKERGETDEFINE:
		return (wParam <= MARKER_MAX)?vs.markers[wParam].markType:0;
	...
	case SCI_MARKERSETALPHA:
		...
		break;
//gg
	case SCI_MARKERGETFORE:
		return (wParam <= MARKER_MAX)?vs.markers[wParam].back.desired.AsLong():0;
//gg
	case SCI_MARKERGETBACK:
		return (wParam <= MARKER_MAX)?vs.markers[wParam].fore.desired.AsLong():0;
	case SCI_MARKERADD: {
...
//gg
	case SCI_SETUSEWHITESPACEFORE:
		vs.whitespaceForegroundSet = wParam != 0;
		InvalidateStyleRedraw();
		break;

//gg
	case SCI_SETUSEWHITESPACEBACK:
		vs.whitespaceBackgroundSet = wParam != 0;
		InvalidateStyleRedraw();
		break;
//gg
	case SCI_GETUSEWHITESPACEFORE:
		return vs.whitespaceForegroundSet;

//gg
	case SCI_GETUSEWHITESPACEBACK:
		return vs.whitespaceBackgroundSet;

//gg
	case SCI_GETWHITESPACEFORE:
		return vs.whitespaceForeground.desired.AsLong();

//gg
	case SCI_GETWHITESPACEBACK:
		return vs.whitespaceBackground.desired.AsLong();
	...
	//gg
	case SCI_SETSCROLLTICK:
		{
			int ret=timer.tickSize;
			timer.tickSize=wParam;
			return ret;
		}
		break;
	//gg
	case SCI_GETSCROLLTICK:
		return timer.tickSize;
	//gg
	case SCI_SETASCROLLDELAY:
		{
			int ret=autoScrollDelay;
			autoScrollDelay=wParam;
			return ret;
		}
	//gg
	case SCI_GETASCROLLDELAY:
