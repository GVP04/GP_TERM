...
cScPoint cScPoint::FromLong(long lpoint) {
	return cScPoint(static_cast<short>(LOWORD(lpoint)), static_cast<short>(HIWORD(lpoint)));
}
...
void cScFont::Create(const char *faceName, int characterSet, int size,
...
public:
	...
	void Polygon(cScPoint *pts, int npts, ColourAllocated fore, ColourAllocated back);
	...
	void Copy(PRectangle rc, cScPoint from, Surface &surfaceSource);

	void DrawTextCommon(PRectangle rc, cScFont &font_, int ybase, const char *s, int len, UINT fuOptions);
	void DrawTextNoClip(PRectangle rc, cScFont &font_, int ybase, const char *s, int len, ColourAllocated fore, ColourAllocated back);
	void DrawTextClipped(PRectangle rc, cScFont &font_, int ybase, const char *s, int len, ColourAllocated fore, ColourAllocated back);
	void DrawTextTransparent(PRectangle rc, cScFont &font_, int ybase, const char *s, int len, ColourAllocated fore);
	void MeasureWidths(cScFont &font_, const char *s, int len, int *positions);
	int WidthText(cScFont &font_, const char *s, int len);
	int WidthChar(cScFont &font_, char ch);
	int Ascent(cScFont &font_);
	int Descent(cScFont &font_);
	int InternalLeading(cScFont &font_);
	int ExternalLeading(cScFont &font_);
	int Height(cScFont &font_);
	int AverageCharWidth(cScFont &font_);
...
void SurfaceImpl::Release() {
	...
	brush = 0;
	if (fontOld) {
		// Fonts are not deleted as they are owned by a cScFont object
...
void SurfaceImpl::SetFont(cScFont &font_) {
	if (font_.GetID() != font) {
		if (fontOld) {
...
void SurfaceImpl::Polygon(cScPoint *pts, int npts, ColourAllocated fore, ColourAllocated back) {
	PenColour(fore);
	BrushColor(back);
...
void SurfaceImpl::AlphaRectangle(PRectangle rc, int cornerSize, ColourAllocated fill, int alphaFill,
	...
	if (AlphaBlendFn && rc.Width() > 0) {
		...
		HBITMAP hbmOld = (HBITMAP)::SelectObject(hMemDC, (HGDIOBJ)hbmMem);
...
void SurfaceImpl::Copy(PRectangle rc, cScPoint from, Surface &surfaceSource) {
	::BitBlt(hdc,
...
void SurfaceImpl::DrawTextCommon(PRectangle rc, cScFont &font_, int ybase, const char *s, int len, UINT fuOptions) {
	SetFont(font_);
	RECT rcw = RectFromPRectangle(rc);
	SIZE sz={0,0};
	int pos = 0;
...
void SurfaceImpl::DrawTextNoClip(PRectangle rc, cScFont &font_, int ybase, const char *s, int len,
...
void SurfaceImpl::DrawTextClipped(PRectangle rc, cScFont &font_, int ybase, const char *s, int len,
	ColourAllocated fore, ColourAllocated back) {
	::SetTextColor(hdc, fore.AsLong());
	::SetBkColor(hdc, back.AsLong());
...
void SurfaceImpl::DrawTextTransparent(PRectangle rc, cScFont &font_, int ybase, const char *s, int len,
...
int SurfaceImpl::WidthText(cScFont &font_, const char *s, int len) {
	SetFont(font_);
	SIZE sz={0,0};
	if ((!unicodeMode) && (IsNT() || (codePage==0) || win9xACPSame)) {
...
void SurfaceImpl::MeasureWidths(cScFont &font_, const char *s, int len, int *positions) {
	SetFont(font_);
	SIZE sz={0,0};
	int fit = 0;
	if (unicodeMode) {
...
int SurfaceImpl::WidthChar(cScFont &font_, char ch) {
	SetFont(font_);
	SIZE sz;
...
int SurfaceImpl::Ascent(cScFont &font_) {
	SetFont(font_);
	TEXTMETRIC tm;
...
int SurfaceImpl::Descent(cScFont &font_) {
	SetFont(font_);
	TEXTMETRIC tm;
	::GetTextMetrics(hdc, &tm);
	return tm.tmDescent;
...
int SurfaceImpl::InternalLeading(cScFont &font_) {
	SetFont(font_);
	TEXTMETRIC tm;
	::GetTextMetrics(hdc, &tm);
...
int SurfaceImpl::ExternalLeading(cScFont &font_) {
	SetFont(font_);
	TEXTMETRIC tm;
	::GetTextMetrics(hdc, &tm);
...
int SurfaceImpl::Height(cScFont &font_) {
	SetFont(font_);
	TEXTMETRIC tm;
	::GetTextMetrics(hdc, &tm);
	return tm.tmHeight;
...
int SurfaceImpl::AverageCharWidth(cScFont &font_) {
	SetFont(font_);
	TEXTMETRIC tm;
	::GetTextMetrics(hdc, &tm);
...
void Window::SetFont(cScFont &font) {
...
PRectangle Window::GetMonitorRect(cScPoint pt) {
...
#else
PRectangle Window::GetMonitorRect(cScPoint) {
	return PRectangle();
}
#endif
...
class ListBoxX : public ListBox {
	...
	cScPoint dragOffset;
	cScPoint location;	// Caret location at which the list is opened
	...
	cScPoint GetClientExtent() const;
	cScPoint MinTrackSize() const;
	cScPoint MaxTrackSize() const;
	...
	static const cScPoint ItemInset;	// Padding around whole item
	static const cScPoint TextInset;	// Padding around text
	static const cScPoint ImageInset;	// Padding around image
...
public:
	...
	virtual void SetFont(cScFont &font);
	virtual void Create(Window &parent, int ctrlID, cScPoint location_, int lineHeight_, bool unicodeMode_);
...
const cScPoint ListBoxX::ItemInset(0, 0);
const cScPoint ListBoxX::TextInset(2, 0);
const cScPoint ListBoxX::ImageInset(1, 0);
...
void ListBoxX::Create(Window &parent_, int ctrlID_, cScPoint location_, int lineHeight_, bool unicodeMode_) {
	parent = &parent_;
	ctrlID = ctrlID_;
	location = location_;
	...
	HINSTANCE hinstanceParent = (HINSTANCE)::GetWindowLong(hwndParent,GWL_HINSTANCE);
...
void ListBoxX::SetFont(cScFont &font) {
	LOGFONT lf;
...
PRectangle ListBoxX::GetDesiredRect() {
	...
	HDC hdc = ::GetDC(lb);
	HFONT oldFont = (HFONT)::SelectObject(hdc, (HGDIOBJ)fontCopy);
	SIZE textSize = {0, 0};
...
void ListBoxX::Draw(DRAWITEMSTRUCT *pDrawItem) {
	if ((pDrawItem->itemAction == ODA_SELECT) || (pDrawItem->itemAction == ODA_DRAWENTIRE)) {
		...
		RECT rcText = rcBox;
		InflateRect(&rcText, -TextInset.x, -TextInset.y);
...
cScPoint ListBoxX::MinTrackSize() const {
	PRectangle rc(0, 0, MinClientWidth(), ItemHeight());
	AdjustWindowRect(&rc);
	return cScPoint(rc.Width(), rc.Height());
}

cScPoint ListBoxX::MaxTrackSize() const {
	PRectangle rc(0, 0,
...
void ListBoxX::ResizeToCursor() {
	...
	cScPoint ptMin = MinTrackSize();
	cScPoint ptMax = MaxTrackSize();
...
cScPoint ListBoxX::GetClientExtent() const {
...
void ListBoxX::CentreItem(int n) {
	...
	if (n >= 0) {
		cScPoint extent = GetClientExtent();
...
void ListBoxX::Paint(HDC hDC) {
	...
	HBITMAP hBitmapOld = (HBITMAP)::SelectObject(bitmapDC, (HGDIOBJ)hBitmap);
	...
	::SelectObject(bitmapDC, GetStockObject(WHITE_BRUSH));
	::SelectObject(bitmapDC, (HGDIOBJ)hBitmapOld);
	::DeleteDC(bitmapDC);
...
LRESULT ListBoxX::WndProc(HWND hWnd, UINT iMessage, WPARAM wParam, LPARAM lParam) {
	switch (iMessage) {
	case WM_CREATE: {
			HINSTANCE hinstanceParent = (HINSTANCE)::GetWindowLong(reinterpret_cast<HWND>(parent->GetID()),GWL_HINSTANCE);
	...
	case WM_GETMINMAXINFO: {
			...
			*reinterpret_cast<cScPoint*>(&minMax->ptMaxTrackSize) = MaxTrackSize();
			*reinterpret_cast<cScPoint*>(&minMax->ptMinTrackSize) = MinTrackSize();
		}
		break;
...
void Menu::Show(cScPoint pt, Window &w) {
