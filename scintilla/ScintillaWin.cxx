...
class ScintillaWin :
	...
	virtual bool DragThreshold(cScPoint ptStart, cScPoint ptNow);
	...
	virtual void NotifyDoubleClick(cScPoint pt, bool shift, bool ctrl, bool alt);
	virtual void Copy();
...
ScintillaWin::ScintillaWin(HWND hwnd) {
	...
	cfLineSelect = static_cast<CLIPFORMAT>(
		::RegisterClipboardFormat("MSDEVLineSelect"));
...
bool ScintillaWin::DragThreshold(cScPoint ptStart, cScPoint ptNow) {
	...
	if (lParam & GCS_RESULTSTR) {
    	...
    	if (hIMC) {
  		...
  		// Set new position after converted
    		cScPoint pos = LocationFromPosition(currentPos);
...
sptr_t ScintillaWin::WndProc(unsigned int iMessage, uptr_t wParam, sptr_t lParam) {
    ...
    case WM_LBUTTONDOWN: {
    	...
    	ButtonDown(cScPoint::FromLong(lParam), ::GetMessageTime(),
    		(wParam & MK_SHIFT) != 0,
    		(wParam & MK_CONTROL) != 0,
    		Platform::IsKeyDown(VK_MENU));
    	::SetFocus(MainHWND());
    ...
    case WM_MOUSEMOVE:
    	ButtonMove(cScPoint::FromLong(lParam));
    	break;

    case WM_LBUTTONUP:
    	ButtonUp(cScPoint::FromLong(lParam),
    		::GetMessageTime(),
    		(wParam & MK_CONTROL) != 0);
    	break;

	case WM_RBUTTONDOWN:
		if (!PointInSelection(cScPoint::FromLong(lParam)))
			SetEmptySelection(PositionFromLocation(cScPoint::FromLong(lParam)));
		break;
    ...
    case WM_SETCURSOR:
    	if (LoWord(lParam) == HTCLIENT) {
			if (inDragDrop == ddDragging) {
    			...
    			if (PointInSelMargin(cScPoint(pt.x, pt.y))) {
    				DisplayCursor(Window::cursorReverseArrow);
				} else if (PointInSelection(cScPoint(pt.x, pt.y)) && !SelectionEmpty()) {
    				DisplayCursor(Window::cursorArrow);
    			} else if (PointIsHotspot(cScPoint(pt.x, pt.y))) {
    ...
    case EM_EXGETSEL: {
    		...
    		cScCharacterRange *pCR = reinterpret_cast<cScCharacterRange *>(lParam);
...
void ScintillaWin::NotifyDoubleClick(cScPoint pt, bool shift, bool ctrl, bool alt) {
...
void ScintillaWin::CreateCallTipWindow(PRectangle) {
    ...
    if (!ct.wCallTip.Created()) {
  	ct.wCallTip = ::CreateWindow(callClassName, "ACallTip",
    					...
    					MainHWND(), 0,
					     (HINSTANCE)::GetWindowLong(MainHWND(),GWL_HINSTANCE),
    					 this);
...
STDMETHODIMP ScintillaWin::DragOver(DWORD grfKeyState, POINTL pt, PDWORD pdwEffect) {
	...
	*pdwEffect = EffectFromState(grfKeyState);
    ...
    SetDragPosition(PositionFromLocation(cScPoint(rpt.x, rpt.y)));
...
STDMETHODIMP ScintillaWin::Drop(LPDATAOBJECT pIDataSource, DWORD grfKeyState,
	...
	*pdwEffect = EffectFromState(grfKeyState);
    ...
    POINT rpt = {pt.x, pt.y};
    ::ScreenToClient(MainHWND(), &rpt);
    int movePos = PositionFromLocation(cScPoint(rpt.x, rpt.y));
...
sptr_t PASCAL ScintillaWin::CTWndProc(
    ...
    if (sciThis == 0) {
  	...
  	if (iMessage == WM_NCDESTROY) {
    		...
    		sciThis->ct.MouseClick(cScPoint(pt.x, pt.y));
    		sciThis->CallTipClick();
    		return 0;
    	} else if (iMessage == WM_LBUTTONDOWN) {
    		// This does not fire due to the hit test code
    		sciThis->ct.MouseClick(cScPoint::FromLong(lParam));
    		sciThis->CallTipClick();
    		return 0;
