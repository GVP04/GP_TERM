 ...
 * cScPoint is exactly the same as the Win32 POINT and GTK+ GdkPoint so can be used interchangeably.
 */
class cScPoint {
public:
	int x;
	...
	explicit cScPoint(int x_=0, int y_=0) : x(x_), y(y_) {
	}

	// Other automatically defined methods (assignment, copy constructor, destructor) are fine

	static cScPoint FromLong(long lpoint);
};
...
protected:
	...
	int ascent;
#endif
	// Private so cScFont objects can not be copied
	cScFont(const cScFont &) {}
	cScFont &operator=(const cScFont &) { id=0; return *this; }
public:
	cScFont();
	virtual ~cScFont();
...
public:
	...
	virtual void Polygon(cScPoint *pts, int npts, ColourAllocated fore, ColourAllocated back)=0;
	...
	virtual void Copy(PRectangle rc, cScPoint from, Surface &surfaceSource)=0;

	virtual void DrawTextNoClip(PRectangle rc, cScFont &font_, int ybase, const char *s, int len, ColourAllocated fore, ColourAllocated back)=0;
	virtual void DrawTextClipped(PRectangle rc, cScFont &font_, int ybase, const char *s, int len, ColourAllocated fore, ColourAllocated back)=0;
	virtual void DrawTextTransparent(PRectangle rc, cScFont &font_, int ybase, const char *s, int len, ColourAllocated fore)=0;
	virtual void MeasureWidths(cScFont &font_, const char *s, int len, int *positions)=0;
	virtual int WidthText(cScFont &font_, const char *s, int len)=0;
	virtual int WidthChar(cScFont &font_, char ch)=0;
	virtual int Ascent(cScFont &font_)=0;
	virtual int Descent(cScFont &font_)=0;
	virtual int InternalLeading(cScFont &font_)=0;
	virtual int ExternalLeading(cScFont &font_)=0;
	virtual int Height(cScFont &font_)=0;
	virtual int AverageCharWidth(cScFont &font_)=0;
...
public:
	...
	PRectangle GetMonitorRect(cScPoint pt);
#if PLAT_MACOSX
...
public:
	...
	virtual void SetFont(cScFont &font)=0;
	virtual void Create(Window &parent, int ctrlID, cScPoint location, int lineHeight_, bool unicodeMode_)=0;
...
public:
	...
	void CreatePopUp();
	void Destroy();
	void Show(cScPoint pt, Window &w);
};
