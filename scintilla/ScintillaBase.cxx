...
//gg
	cScPoint pt = LocationFromPosition(currentPos - lenEntered);
...
//gg
void ScintillaBase::CallTipShow(cScPoint pt, const char *defn) {
	ac.Cancel();
	pt.y += vs.lineHeight;
...
//gg
void ScintillaBase::ContextMenu(cScPoint pt) {
	if (displayPopupMenu) {
...
//gg
void ScintillaBase::ButtonDown(cScPoint pt, unsigned int curTime, bool shift, bool ctrl, bool alt) {
	CancelModes();
...
//gg
			keyWordLists[wParam]->Set(lParam?reinterpret_cast<const char *>(lParam):"");
		}
		break;

//gg
	case SCI_GETKEYWORDS:
		if (wParam < numWordLists && lParam) {
			keyWordLists[wParam]->Get((char **)lParam);
		}
		break;
//gg
	case SCI_GETKEYWORDSCOUNT:
		return numWordLists;
		break;
