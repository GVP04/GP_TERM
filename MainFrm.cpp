...
CMainFrame::CMainFrame()
{
	...
	mIconAudioOn=NULL;
	mIconAudioOff=NULL;
	mIconAudioOffActive=NULL;
}

CMainFrame::~CMainFrame()
{
	if (mIconAudioOn		) DestroyIcon(mIconAudioOn		); 
	if (mIconAudioOff		) DestroyIcon(mIconAudioOff		); 
	if (mIconAudioOffActive	) DestroyIcon(mIconAudioOffActive); 
	if (FRDlg) delete FRDlg; 
...
void CMainFrame::OnUpdateKeyblock(CCmdUI* pCmdUI) 
{
	...
	try
	{
		...
		if (GPC.m_Document && (s=GPC.m_Document->GetActiveScreen()) && (w=s->GPActiveWindow()))
		{
			...
			m_wndStatusBar.SetPaneText(m_wndStatusBar.CommandToIndex(ID_PROTECTFIELDS),ccTxt);
		}
	}catch(...)
	{
	...
	try
	{
		...
		if (!mIconAudioOn)  mIconAudioOn	= (HICON)LoadImage(AfxGetInstanceHandle(),MAKEINTRESOURCE(IDI_ICONAUDIOON),IMAGE_ICON,16,16,0);
		if (!mIconAudioOff) mIconAudioOff	= (HICON)LoadImage(AfxGetInstanceHandle(),MAKEINTRESOURCE(IDI_ICONAUDIOOFF),IMAGE_ICON,16,16,0);	
		if (!mIconAudioOffActive) mIconAudioOffActive	= (HICON)LoadImage(AfxGetInstanceHandle(),MAKEINTRESOURCE(IDI_ICONAUDIOOFFACTIVE),IMAGE_ICON,16,16,0);	
		if (GPC.m_View)
		{
			if ((curIcon=GPC.m_View->m_uLockBeep?((GPC.m_View->m_uLockBeep&0x2)?mIconAudioOffActive:mIconAudioOff):mIconAudioOn)!=m_wndStatusBar.GetStatusBarCtrl().GetIcon(m_wndStatusBar.CommandToIndex(ID_SOUNDONOFF)))
				m_wndStatusBar.GetStatusBarCtrl().SetIcon(m_wndStatusBar.CommandToIndex(ID_SOUNDONOFF),curIcon);
			if ((GPC.m_View->m_uLockBeep&0x6)) GPC.m_View->m_uLockBeep-=0x2;
		}
	}catch(...)
	{
		GPC.m_ErrReport.GPWriteErrorLog("CMainFrame", "OnUpdateKeyblock 815","");
	}
}
	...
	if (pnmh)
	{
		...
		case NM_CLICK:
			if (pnmh->hwndFrom==m_wndStatusBar.m_hWnd)
			{
				UINT dwStyle;
				int dwWidth;
				UINT dwCase=lpn->dwItemSpec;
				if (lpn->dwItemSpec<100) m_wndStatusBar.GetPaneInfo(lpn->dwItemSpec,dwCase,dwStyle,dwWidth);
				...
				case ID_SEPARATOR:
					break;
				case ID_SOUNDONOFF: 
					if (GPC.m_View) GPC.m_View->m_uLockBeep=(GPC.m_View->m_uLockBeep)?0:1;
					AfxGetApp()->WriteProfileInt(GPC.cs_RegTermViewSetName, "m_uLockBeep",GPC.m_View->m_uLockBeep);
					break;
				case ID_INDICATOR_OVR:
					break;
				case ID_PROTECTFIELDS:
					try
					{
						GPScreen *s;
						GPWindow *w;
