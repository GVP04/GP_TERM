...
BEGIN_MESSAGE_MAP(CGP_TermView, CFormView)
	...
	ON_COMMAND(ID_SCRIPTEXECUTE, OnScriptExecute)
	ON_UPDATE_COMMAND_UI(ID_SCRIPTEXECUTE, OnUpdateScriptExecute)
	ON_COMMAND(ID_SCRIPTSTOP, OnScriptStop)
	ON_UPDATE_COMMAND_UI(ID_SCRIPTSTOP, OnUpdateScriptStop)
...
//
	...
	m_uLockBeep=AfxGetApp()->GetProfileInt(GPC.cs_RegTermViewSetName, "m_uLockBeep",0);
	...
	case CURSOR_TIMER_ID:
		{
			...
			if (s->GetCursorState())
			{
				...
				if (mdc)
				{
					...
					if (!ocontext || ocontext->m_ContextName!=ccName)
					{
						if (!ocontext) ocontext=m_ODContextsArray->CreateContext(ccName);
...
void CGP_TermView::OnKeyDown(UINT nChar, UINT nRepCnt, UINT nFlags)
{
	...
	if ((nFlags==0x0001 || nFlags==0x4001)  && nChar==27 && next_job==-53 && forcounter>3) forcounter=123456;
	else
...
//#endif
	...
	m_ODContextsArray->pTerminalUserMenuContext=m_ODContextsArray->LoadContext("CustomMenu", "");
...
void CGP_TermView::OnUpdateScriptStop(CCmdUI* pCmdUI)
{
	pCmdUI->Enable(GPC.cs_ScriptLine!="");
}
void CGP_TermView::OnUpdateScriptExecute(CCmdUI* pCmdUI)
{
	pCmdUI->Enable(GPC.cs_ScriptLine=="");
}

void CGP_TermView::OnScriptStop()
{
	KillTimer(SCRIPTOUT_TIMER_ID);
	GPC.cs_ScriptLine="";
	GPC.cs_ScriptWaitLineCurrent="";
	GPC.dw_ScriptWaitFlag=0;
	GPC.m_GPScript.Reset();
}

void CGP_TermView::OnScriptExecute()
{
	OnScriptStop();
	OPENFILENAME opf={0};
	DWORD i;
	for(i=0;i<GP_NFILES && aGP_File[i]!=NULL;i++)
		if (aGP_File[i])
		{
			delete aGP_File[i];
			aGP_File[i]=NULL;
	...
	*opf.lpstrFile=0;
	opf.nMaxFile =4900;
	opf.Flags =OFN_ENABLESIZING|OFN_EXPLORER|OFN_FILEMUSTEXIST|OFN_LONGNAMES|OFN_PATHMUSTEXIST|OFN_NOCHANGEDIR|OFN_SHOWHELP;
	#ifdef RUS
		opf.lpstrFilter="Файл скрипта терминала(*.BTSF)\x0*.BTSF\x0";
		opf.lpstrTitle="Чтение файла скрипта терминала";
	#else
		opf.lpstrFilter="Terminal script File (*.BTSF)\x0*.BTSF\x0";
		opf.lpstrTitle="Load Terminal Script File";
	#endif

	opf.lpstrDefExt="BTSF";
	...
	if (GetOpenFileName(&opf))
	{
		...
		GPC.m_GPScript.csLoadPath=m_csLastDirLoad;
		if (GPC.m_GPScript.LoadScript(opf.lpstrFile,CGPScript::GPScriptLoadOptionDefault)>0)
		{
			EnterCriticalSection(&(GPC.m_View->m_CSRemap));
			GPC.cs_ScriptLine=GPC.m_GPScript.GetNextLine();
			if (GPC.cs_ScriptLine!="") DoScriptCommand();
...
DWORD CGP_TermView::DoScriptCommand()
{
	...
	while(bContinue)
	{
						...
						case 'q': //query
							{
								...
								if (csSendStr!="")
								{
									...
									GPC.cs_ScriptLine=GPC.cs_ScriptLine.Mid(pos-posstrt-1);
									posstrt=pos=GPC.cs_ScriptLine;
									pos++;
								}
								switch(*pos)
								{
		...
		if (GPC.cs_ScriptLine=="" && dwDelay!=0xFFFFFFFF)
			GPC.cs_ScriptLine=GPC.m_GPScript.GetNextLine();

		if (GPC.cs_ScriptLine!="" && dwDelay!=0xFFFFFFFF)
		{
			int pos22=GPC.cs_ScriptLine.Find("\\s");
			if (pos22>=0 && pos22<2) 
				bContinue=TRUE;
			else 
				SetTimer(SCRIPTOUT_TIMER_ID,dwDelay,NULL);
		}
	}
...
void CGP_TermView::UpdateMenuByAdm()
{
	...
	if (m_GPMenu && m_GPMenu->m_ArrPopupMenu[0])
	{
		...
		TRACE("CGP_TermView::UpdateMenuByAdm()\n");
	}
#endif
}
