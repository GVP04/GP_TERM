...
#if !defined(AFX_EXCEL8_H__1509E7B2_DD4C_11D3_970D_0080AD863777__INCLUDED_)
#define AFX_EXCEL8_H__1509E7B2_DD4C_11D3_970D_0080AD863777__INCLUDED_

#define N_EXCELWORKBOOK_ARR		20
#define N_EXCELWORKSHEET_ARR		50

/////////////////////////////////////////////////////////////////////////////
// GPExcelOLEWorksheet wrapper class

class GPExcelOLEWorksheet : public COleDispatchDriver
{
...
public:
	...
	GPExcelOLEWorksheet(LPDISPATCH pDispatch) : COleDispatchDriver(pDispatch) {}
	GPExcelOLEWorksheet(const GPExcelOLEWorksheet& dispatchSrc) : COleDispatchDriver(dispatchSrc) {}
...
public:
	...
	LPDISPATCH GetAutoFilter();
	  // insert function's
	int DoMethod(const char* iMethod,char* oStr,BOOL flgRet);
	LPDISPATCH CreateGPExcelRange(const char *iStr);
	int PrintOutEx(const char* iStr);  
	int ProtectEx(const char* iStr);
	int SaveAsEx(const char* iStr);
	int PasteSpecialEx(const char* iStr);
};

/////////////////////////////////////////////////////////////////////////////
// _Workbook wrapper class

class GPExcelOLEWorkbook : public COleDispatchDriver
{
friend class CGP_TermDoc;
public:
	GPExcelOLEWorkbook(); 
	GPExcelOLEWorkbook(LPDISPATCH pDispatch);
	GPExcelOLEWorkbook(const GPExcelOLEWorkbook& dispatchSrc) : COleDispatchDriver(dispatchSrc) {}
	~GPExcelOLEWorkbook(); 
// Attributes
public:
...
public:
	...
	int RefreshGPExcelWorksheets();
	int DoMethod(const char* iMethod,char* oStr,BOOL flgRet);
	int DoWsMethod(int IdWorksheet,const char* iMethod,char* oStr,BOOL flgRet);
	int SaveAsEx(const char *iStr);
	int CloseEx(const char* iStr);
	int PrintOutEx(const char* iStr);
	int ProtectEx(const char* iStr);
	int ProtectSharingEx(const char* iStr);
	int HighlightChangesOptionsEx(const char* iStr);
	int PurgeChangeHistoryNowEx(const char* iStr);
	int MyIdWorkbook;
	int MyIdApp;
private:
	GPExcelOLEWorksheet* gpWorksheets[N_EXCELWORKSHEET_ARR];
};

class GPExcelOLEApplication : public COleDispatchDriver
{
friend class CGP_TermDoc;
public:
	GPExcelOLEApplication() {
		MyIdApp=-1;
		int i=0;
	...
	GPExcelOLEApplication(LPDISPATCH pDispatch) : COleDispatchDriver(pDispatch) 
	{
		MyIdApp=-1;
		int i=0;
		while (i<N_EXCELWORKBOOK_ARR)
	...
	GPExcelOLEApplication(const GPExcelOLEApplication& dispatchSrc) : COleDispatchDriver(dispatchSrc) 
	{
		MyIdApp=-1;
		int i=0;
		while (i<N_EXCELWORKBOOK_ARR)
...
public:
	LPDISPATCH GetApplication();
	long GetCreator();
	LPDISPATCH GetParent();  // return application
	LPDISPATCH GetActiveCell(); // return dispatch for range
	LPDISPATCH GetActiveChart(); //return chart
	CString GetActivePrinter();  // return printer
	void SetActivePrinter(LPCTSTR lpszNewValue);
	LPDISPATCH GetActiveSheet();  //return sheet
	LPDISPATCH GetActiveWindow();
	LPDISPATCH GetActiveWorkbook();
	LPDISPATCH GetAddIns();   // return addins
	LPDISPATCH GetAssistant();
	void Calculate();
	LPDISPATCH GetCells();  //return range
	LPDISPATCH GetCharts();
	LPDISPATCH GetColumns();
	LPDISPATCH GetCommandBars();
	long GetDDEAppReturnCode();
	void DDEExecute(long Channel, LPCTSTR String); // dynamic data exchange
	long DDEInitiate(LPCTSTR App, LPCTSTR Topic);  // dynamic data exchange
	void DDEPoke(long Channel, const VARIANT& Item, const VARIANT& Data);  // dynamic data exchange
	VARIANT DDERequest(long Channel, LPCTSTR Item); // dynamic data exchange
	void DDETerminate(long Channel);  // dynamic data exchange
	VARIANT Evaluate(const VARIANT& Name);  // from name to object
	VARIANT _Evaluate(const VARIANT& Name);	// from name to object
	VARIANT ExecuteExcel4Macro(LPCTSTR String); //exec macro in ...
	LPDISPATCH Intersect(LPDISPATCH Arg1, LPDISPATCH Arg2, const VARIANT& Arg3, const VARIANT& Arg4, const VARIANT& Arg5, const VARIANT& Arg6, const VARIANT& Arg7, const VARIANT& Arg8, const VARIANT& Arg9, const VARIANT& Arg10, 
		...
		const VARIANT& Arg21, const VARIANT& Arg22, const VARIANT& Arg23, const VARIANT& Arg24, const VARIANT& Arg25, const VARIANT& Arg26, const VARIANT& Arg27, const VARIANT& Arg28, const VARIANT& Arg29, const VARIANT& Arg30);  // intersect of range's
	LPDISPATCH GetNames(); //return names of active workbook
	LPDISPATCH GetRange(const VARIANT& Cell1, const VARIANT& Cell2); //return range
	LPDISPATCH GetRows();
	...
	void SendKeys(const VARIANT& Keys, const VARIANT& Wait); // push button's
	LPDISPATCH GetSheets();  //return sheet's
	LPDISPATCH GetThisWorkbook();  // return workbook where macro running
	LPDISPATCH Union(LPDISPATCH Arg1, LPDISPATCH Arg2, const VARIANT& Arg3, const VARIANT& Arg4, const VARIANT& Arg5, const VARIANT& Arg6, const VARIANT& Arg7, const VARIANT& Arg8, const VARIANT& Arg9, const VARIANT& Arg10, const VARIANT& Arg11, 
		...
		const VARIANT& Arg22, const VARIANT& Arg23, const VARIANT& Arg24, const VARIANT& Arg25, const VARIANT& Arg26, const VARIANT& Arg27, const VARIANT& Arg28, const VARIANT& Arg29, const VARIANT& Arg30); // union of range's
	LPDISPATCH GetWindows();
	LPDISPATCH GetWorkbooks();
	LPDISPATCH GetWorksheetFunction(); // ?!
	LPDISPATCH GetWorksheets();
	LPDISPATCH GetExcel4IntlMacroSheets(); // return international macro's
	LPDISPATCH GetExcel4MacroSheets(); // return sheet's with macro's
	void ActivateMicrosoftApp(long Index); //activate app
	void AddChartAutoFormat(const VARIANT& Chart, LPCTSTR Name, const VARIANT& Description); // add new autoformat
	void AddCustomList(const VARIANT& ListArray, const VARIANT& ByRow); // add custom list
	BOOL GetAlertBeforeOverwriting();
	void SetAlertBeforeOverwriting(BOOL bNewValue);
	CString GetAltStartupPath();
	void SetAltStartupPath(LPCTSTR lpszNewValue);  // path of application
	...
	int CloseGPExcelWorksheet(const char *iStr);
	int DoMethod(const char* iMethod,char* oStr,BOOL flgRet);
	int DoWbMethod(int IdWorkBook,const char* iMethod,char* oStr,BOOL flgRet);
	int DoWsMethod(int IdWorkbook, const char *iMethod, char *oStr,BOOL flgRet); // Worksheet Method
	VARIANT RunEx(const char* iStr); //run macro
	void SendKeysEx(const char* iStr);
	BOOL CheckSpellingEx(const char* iStr);
	VARIANT GetOpenFilenameEx(const char* iStr);
	VARIANT GetSaveAsFilenameEx(const char* iStr);
	VARIANT InputBoxEx(const char* iStr);
	int MacroOptionsEx(const char* iStr);
	int MailLogonEx(const char* iStr);
	int OnTimeEx(const char* iStr);
	int MyIdApp;
...
private:
	GPExcelOLEWorkbook* gpWorkbooks[N_EXCELWORKBOOK_ARR];
};
...
class GPExcelOLERange : public COleDispatchDriver
{
friend class CGP_TermDoc;
public:
	GPExcelOLERange() {}		// Calls COleDispatchDriver default constructor
	GPExcelOLERange(LPDISPATCH pDispatch) : COleDispatchDriver(pDispatch) {}
	GPExcelOLERange(const GPExcelOLERange& dispatchSrc) : COleDispatchDriver(dispatchSrc) {}
...
public:
	...
	//insert function's
	int DoMethod(const char* iMethod,char* oStr,BOOL flgRet);
	CString GetAddressEx(const char* iStr);
	CString GetAddressLocalEx(const char* iStr);
	int AdvancedFilterEx(const char* iStr);
	int ApplyNamesEx(const char* iStr);
	int AutoFilterEx(const char* iStr);
	int AutoFormatEx(const char* iStr);
	int BorderAroundEx(const char* iStr);
	int ConsolidateEx(const char* iStr);
	int CreateNamesEx(const char* iStr);
	int CreatePublisherEx(const char* iStr);
	int DataSeriesEx(const char* iStr);
	VARIANT EditionOptionsEx(const char* iStr);
	int GroupEx(const char* iStr);
	int NavigateArrowEx(const char* iStr);
	CString NoteTextEx(const char* iStr);
	int PasteSpecialEx(const char* iStr);
	int PrintOutEx(const char* iStr);
	BOOL ReplaceEx(const char* iStr);
	VARIANT RunEx(const char* iStr);
	int SortEx(const char* iStr);
	int SortSpecialEx(const char* iStr);
	int SubtotalEx(const char* iStr);
	int BordersEx(const char* iStr);
	int FontEx(const char* iStr);
};
/////////////////////////////////////////////////////////////////////////////
...
#endif AFX_EXCEL8_H__1509E7B2_DD4C_11D3_970D_0080AD863777__INCLUDED_