...
BOOL GPMenu::GPAppendMenu(UINT IdPopup, UINT IdItem, char *posflg, char *postxt)
{
	...
	if (m_ArrPopupMenu[IdPopup]!=NULL)	
	{
		...
		MII.dwItemData=RegisterMenuItem(IdPopup,IdItem,csTmpContext,tmpMenuText,m_ArrPopupMenu[IdPopup]->m_dwViewType);
...
DWORD GPMenu::RegisterMenuItem(DWORD dwPopupId, DWORD dwItemId, const char *ccContext, const char *Text, DWORD dwFlags)
{
...
DWORD GPMenu::SetODCForTerminalMenu(CODContext *ipContext, HMENU ihPopup, DWORD dwFlags)
{
	...
	if (ipContext && ipContext->m_ContextName==GPC.csODCTerminalMenuName
		...
		for(pos=::GetMenuItemCount(ihPopup)-1;pos>=0;pos--)
		{
			...
			*tmpPosD=0;
			mii.dwItemData=RegisterMenuItem(((DWORD)ihPopup)|0x80000000,(UINT)mii.wID, GPC.csODCTerminalMenuName, menuText2, dwFlags);
			tmpPos=menuText;
			while(*tmpPos)
			{
