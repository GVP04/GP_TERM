		...
		if (GPC.dw_ScriptWaitFlag)
		{
			if (GPC.cs_ScriptWaitLine!="")
			{
				...
				*tmadd=InputBuffer[i];
				...
				if (GPC.cs_ScriptWaitLineCurrent.Find(GPC.cs_ScriptWaitLine,0)<0)
				{
					if (GPC.cs_ScriptWaitLineCurrent.GetLength()>GPC.cs_ScriptWaitLine.GetLength())
						GPC.cs_ScriptWaitLineCurrent=GPC.cs_ScriptWaitLineCurrent.Mid(GPC.cs_ScriptWaitLineCurrent.GetLength()-GPC.cs_ScriptWaitLine.GetLength());
				}
				else
				{
					GPC.cs_ScriptWaitLineCurrent="";
