...
/*120*/	"GetTermProcInfo",		"ExecuteFileScript",	"TWAIN",	"",	"",
/*125*/	"",	"",	"",	"",	"",
/*130*/	"",	"",	"",	"",	"",
/*135*/	"",	"",	"",	"",	"",
	};
...
/*115*/	"[X],[y],[CX],[CY]",	"",	"",	"",	"",
/*120*/	"",	"Filename[,WorkingFolder]",	"MethodName[,Param1[,Param2[,Param3...]]]",	"",	"",
/*125*/	"",	"",	"",	"",	"",
/*130*/	"",	"",	"",	"",	"",
/*135*/	"",	"",	"",	"",	"",
	};
...
int CDlgViewSystemMethods::DoMethod(const char *iStr, char *oStr)
{
		...
		*/	
		...
		case 48: /* GetSystemMetrics nIndex[;nIndex...]*/
			if (oStr)
			{
				...
				if (atoipos==-1 || !*pos)
				{
					for(nIndex=0;*chMetricsNames[nIndex];nIndex++)
					{
				...
				{
					CString tmppos;
					for(i=0;(tmppos=ExtractField(pos,i+1,";"))!="";i++)
					{
						int j;
						if ((nIndex=atoi(tmppos))==0 && tmppos[0]!='0')
						{
							for(j=0,nIndex=-1;*chMetricsNames[j] && nIndex==-1;j++)
		...
		case 50: /*GetSysColorBrush nIndex[;nIndex...]*/
			if (oStr)
			{
				...
				if (atoipos==-1 || !*pos)
				{
					for(nIndex=0;*chColorNames[nIndex];nIndex++)
					{
						if (nIndex) *(postxt++)=0x7F;
						*postxt=0;
						LOGBRUSH LogBrush={0};
				...
				else
				{
					...
					{
						int j;
						if (tmppos.Find("COLOR_")<0) tmppos="COLOR_"+tmppos;
						for(j=0;*chColorNames[j] && tmppos!=chColorNames[j];j++);
						nIndex=uColorNames[j];
						if (i) *(postxt++)=0x7F;
						*postxt=0;
						LOGBRUSH LogBrush={0};
						HBRUSH mcb=GetSysColorBrush(nIndex);
						if (mcb)
							CBrush::FromHandle(mcb)->GetLogBrush(&LogBrush);
						GPGetBrushString(&LogBrush,postxt);
						while(*postxt) postxt++;
					}
				}
		...
		case 51: /*SetSysColors nIndex=R,G,B[;nIndex=R,G,B]*/
			{
				...
				int	  a_ind[30];
				int ni=0;
				for(ni=0;(tmppos=ExtractField(pos,ni+1,";"))!="" && ni<29;ni++)
				{
					...
					{
						int i;
						for(i=0,nIndex=-1;*chColorNames[i] && nIndex==-1;i++)
		...
		case 84: /*SetVariable VariableName,Value*/
			{
				...
				if (!*scrPos)
				{
					if (varSpr) GPC.SprVariables.DeleteItem(GPC.SprVariables.uPosition-1);
				}
				else
				{
					if (!varSpr)
		...
		case 83: /*DeleteVariable VarName[,VarName2[,..]]*/
			{
				...
				while((tmpName=ExtractField(pos,++i,","))!="")
					if ((varSpr=(CString *)GPC.SprVariables.FindItemExact(0,tmpName)))
						GPC.SprVariables.DeleteItem(GPC.SprVariables.uPosition-1);
			}
			break;
		...
		case 101: //ProcMonitorSetState IDPROC=[;IDSCR=][;IDDLG=][;STATE=]
			{
				...
				case 98: //ShellExecuteEx [VERB=][;FILE=][;CLASSNANE=][;CLASSKEY=][;DIR=][;SHOW=][;HOTKEY=][;FLAG=][;PARAM=]
					...
					retint=ShellExecuteEx(&ShExecInfo);
					if (ShExecInfo.hProcess)
					{
						retint=procMon.dwProcID=(DWORD)(ShExecInfo.hProcess);
					#ifdef W_95
					#else
						#ifdef W_NT
						#else
								...
								#ifdef W_2000
								#else
						retint=procMon.dwProcID=GetProcessId(ShExecInfo.hProcess);
								#endif
							#endif
						#endif
					#endif
		...
		case 119: //CapturePreviewSetupDialog
			...
			break;
		case 120: //GetTermProcInfo
			if (oStr) CProcMonitor::GetProcessDescription(GetCurrentProcessId(),oStr);
			break;
		case 121: //ExecuteFileScript Filename[,WorkingFolder]
			{
				GPC.m_View->OnScriptStop();
				if (*pos)
				{
					GPC.m_GPScript.csLoadPath=ExtractField(pos,2,",");
					retint=GPC.m_GPScript.LoadScript(ExtractField(pos,1,","),CGPScript::GPScriptLoadOptionDefault);
					GPC.m_View->SetTimer(SCRIPTOUT_TIMER_ID,50,NULL);
				}
			}
			break;
		case 122: //TWAIN MethodName[,Param1[,Param2[,Param3...]]]
			{
				CString csMethodName=ExtractField(pos,1,",");
				csMethodName.MakeUpper();
				const char *twaiMethods[]=
				{
					"INITTWAIN","RELEASETWAIN","SELECTSOURCE","OPENSOURCE","DSOPEN",
					"DSMOPEN","CLOSEDSM","CLOSEDS","SELECTDEFAULTSOURCE","ISVALIDDRIVER",
					"ACQUIRE","GETIMAGEINFO","TRANSFERIMAGE","CANCELTRANSFER","DISABLESOURCE",
					"ENABLESOURCE","SETIMAGECOUNT","SETCAPABILITY","GETCAPABILITY","GETIMAGE",
					"ENDTRANSFER","","","","",
				};

				int iSubMethod;
				for(iSubMethod=0;*twaiMethods[iSubMethod];iSubMethod++);
				if (*twaiMethods[iSubMethod])
				{
					if (!GPC.m_Twain) GPC.m_Twain=new CGPTwain;
					pos=FindField(pos,2,",");

					switch(iSubMethod)
					{
					case 0: //InitTwain[,IdDlg]
						{
							HWND tmpHWND=NULL;
							if (*pos)	tmpHWND=GPC.m_Document->GetDlgHWND(GPC.m_Document->FindDlg(pos));
							if (!tmpHWND) tmpHWND=GPC.m_hViewWnd;
							retint=GPC.m_Twain->InitTwain(tmpHWND);
						}
						break;
					case 1: //ReleaseTwain
						 GPC.m_Twain->ReleaseTwain();
						break;
					case 2: //	SelectSource
						retint=GPC.m_Twain->SelectSource();
						break;
					case 3: //OpenSource
						{
							//TW_IDENTITY mSource;
							//retint=GPC.m_View->OpenSource(&mSource);
							retint=GPC.m_Twain->OpenSource();
						}
						break;
					case 4: //DSOpen
						retint=GPC.m_Twain->DSOpen();
						break;
					case 5: //DSMOpen
						retint=GPC.m_Twain->DSMOpen();
						break;
					case 6: //CloseDSM
						GPC.m_Twain->CloseDSM();
						break;
					case 7: //CloseDS
						GPC.m_Twain->CloseDS();
						break;
					case 8: //SelectDefaultSource
						retint=GPC.m_Twain->SelectDefaultSource();
						break;
					case 9: //IsValidDriver()
						retint=GPC.m_Twain->IsValidDriver();
						break;
					case 10: //Acquire[,COUNT]
						retint=atoi(pos);
						retint=GPC.m_Twain->Acquire(retint?retint:TWCPP_ANYCOUNT);
						break;
					case 11: //GetImageInfo
						if (oStr)
						{
							TW_IMAGEINFO info={0};
							GPC.m_Twain->GetImageInfo(info);
							sprintf(oStr,"%d.%d\x7F%d.%d\x7F%d\x7F%d\x7F%d\x7F%d\x7%d\x7%d\x7%d\x7%d\x7%d\x7%d\x7%d\x7F%d\x7F%d\x7F%d\x7F%d",
								 info.XResolution.Whole,
								 info.XResolution.Frac,
								 info.YResolution.Whole,
								 info.YResolution.Frac,
								 info.ImageWidth,
								 info.ImageLength,
								 info.SamplesPerPixel,
								 info.BitsPerSample[0],info.BitsPerSample[1],info.BitsPerSample[2],info.BitsPerSample[3],info.BitsPerSample[4],info.BitsPerSample[5],info.BitsPerSample[6],info.BitsPerSample[7],
								 info.BitsPerPixel,
								 info.Planar,
								 info.PixelType,
								 info.Compression);
						...
						break;
					case 12: //TransferImage
						GPC.m_Twain->TransferImage();
						break;
					case 13: //CancelTransfer
						GPC.m_Twain->CancelTransfer();
						break;
					case 14: //DisableSource
						retint=GPC.m_Twain->DisableSource();
						break;
					case 15: //EnableSource[,showUI]
						retint=GPC.m_Twain->EnableSource(atoi(pos));
						break;
					case 16: //SetImageCount,nCount
						retint=GPC.m_Twain->SetImageCount(*pos?atoi(pos):1);
						break;
					case 17: //SetCapability,Cap,ConType,hContainer
						{
							TW_CAPABILITY cap={0};
							cap.Cap=atoi(pos);
							cap.ConType=atoi(pos=FindField(pos,2,","));
							cap.hContainer=(TW_HANDLE)atoi(pos=FindField(pos,2,","));
							retint=GPC.m_Twain->SetCapability(cap);
						}
						break;
					case 18: //GetCapability
						if (oStr)
						{
							TW_UINT16 cap=0;
							TW_UINT32 value=0;
							retint=GPC.m_Twain->GetCapability(cap,value);
							sprintf(oStr,"%d\x7F%d",cap,value);
						}
						break;
					case 19: //GetImage
		/*				{
							TW_IMAGEINFO info={0};
							retint=GPC.m_Twain->GetImage(info);
   ...
   TW_UINT16  Compression;     
						...
						break;
					case 20: //EndTransfer()
						retint=GPC.m_Twain->EndTransfer();
						break;
					default:
						break;
					}
