// GPDrLBox.cpp : implementation file
//
...
/////////////////////////////////////////////////////////////////////////////
// GPDrLBox
const UINT GPDrLBox::uNMethods=79;
const char *GPDrLBox::cArrMethods[]={
...
/*35*/		"Dir",				"FindString",		"FindStringExact",		"SelectString",			"GetItemText",
/*40*/		"GetItemTextEx",		"CompareItem",		"DeleteItem",			"VKeyToItem",			"CharToItem",
/*45*/		"DrawInsert",		"ItemFromPt",		"BeginDrag",			"CancelDrag",			"Dragging",
/*50*/		"Dropped",			"GetSelectedItem",	"UpdateGroup",			"GetStyle",				"AddStrings",
/*55*/		"GetContents",		"SelectAll",		"DeSelectAll",			"SelectionDelete",		"SelectionInvert",
/*60*/		"SelectionDoMethod","SetSelection",		"FillFromSpr",			"ReplaceFromSpr",		"FillFromList",
/*65*/		"AddFromSpr",		"AddFromList",		"SetItemText",			"AddFile",				"GetListBoxInfo",
/*70*/		"AddItem",			"DeleteItemEx",		"SetCurSelByItemData",	"SetAutoFocus",			"SetItemDataEx",
/*75*/		"GetItemDataEx",	"AddItemEx",		"SetItemTextEx",		"SetCurSelByItemDataEx","",
/*80*/		"",	"",		"","","", "", "", 
	};

const char *GPDrLBox::cArrMethodsParams[]={
...
/*45*/		"nIndex","poX,poY","Not Present","Not Present","Not Present",
/*50*/		"Not Present","","","","Str1^Str2^...^StrN",
/*55*/		"","","","","",
/*60*/		"Method Params","Sel1,Sel2,...,SelN","SprName,Separator,nField1,nField2...","SprName,IdentField,Separator,nField1,nField2...","ListName",
/*65*/		"SprName,Separator,nField1,nField2...","ListName","[iItem],iItemData;Text","FileNAme","",
/*70*/		"[iBefore],ItemData;ItemText","ItemData","ItemData",	"bAuto","ItemId,ItemDataEx","ItemId",
...
GPDrLBox::GPDrLBox(UINT iUnitId)
{
	ItemDataArray=NULL;
	m_bAutoFocus=TRUE;
	m_LCDOflag=0;
...
BEGIN_MESSAGE_MAP(GPDrLBox, CDragListBox)
	//{{AFX_MSG_MAP(GPDrLBox)
	ON_CONTROL_REFLECT(LBN_SELCHANGE, OnSelchange)
	ON_CONTROL_REFLECT(LBN_ERRSPACE, OnErrspace)
	ON_CONTROL_REFLECT(LBN_SELCANCEL, OnSelcancel)
	ON_CONTROL_REFLECT(DL_BEGINDRAG, OnBEGINDRAG)
...
void GPDrLBox::OnBEGINDRAG()
{
	if (referGPU!=NULL)
			referGPU[UnitId&0xFFFF]->GPMSG->GPSendObjEvent(m_hWnd, UnitId,"OnBeginDrag","");

}

/////////////////////////////////////////////////////////////////////////////
// GPDrLBox message handlers

UINT GPDrLBox::DoMethod(const char *iStr, char *oStr)
{
	char met[150];
	UINT tmpUINT=0;
	...
	case 42: /*DeleteItem ItemData*/
		...
		break;
	case 45: /*DrawInsert*/ DrawInsert(AtoiPos);	break;
	case 46: /*ItemFromPt*/
		if (oStr!=NULL)
		{
			CPoint po;po.x=0;po.y=0; BOOL out=0;
			m_LastScanf=sscanf(pos,"%d,%d",&po.x,&po.y);
			tmpUINT=ItemFromPt(po,out);
			sprintf(oStr,"%d\x7F%d",tmpUINT,out);
		}
		break;
	case 47: /*BeginDrag*/
	case 48: /*CancelDrag*/
	case 49: /*Dragging*/
	case 50: /*Dropped*/	if (oStr!=NULL)	sprintf(oStr,"not released in this version");break;
	case 51: /*GetSelectedItem*/	if (oStr!=NULL)	GetSelectedItem(oStr);break;
	case 52: /*UpdateGroup*/
	...
	case 57: /*DeSelectAll*/
		...
		break;
	case 58: /*SelectionDelete*/
	case 59: /*SelectionInvert*/
	case 60: /*SelectionDoMethod*/
	case 61: /*SetSelection*/
		{
		...
		break;
	case 62: /*FillFromSpr*/  //"SprName,Separator,nField1,nField2..."
	case 65: /*AddFromSpr*/  //"SprName,Separator,nField1,nField2..."
		{
			char *pos333=new char [(int)strlen(pos)+2];
			strcpy(pos333,pos);
			...
			if (spr!=NULL) retint=spr->FillObject(m_hWnd,GPSPRFILL_LISTBOX,tmp2,nfunc==62?0:1);
			delete[] pos333;
		}
		break;
	case 63: /*ReplaceFromSpr*/ //"SprName,IdentField,Separator,nField1,nField2..."
		{
			char *pos333=new char [(int)strlen(pos)+2];
			strcpy(pos333,pos);
			char *tmp=pos333,*tmp2;
		...
		break;
	case 64: /*FillFromList*/ //"ListName"
	case 66: /*AddFromList*/ //"ListName"
		retint=GPC.m_ListMaker.FillObject(pos,m_hWnd,GPSPRFILL_LISTBOX,nfunc==64?0:1);
		break;
	case 67: /*SetItemText*/
		{
	...
	case 70: /*AddItem iBefore,ItemData;ItemText*/
		...
		break;
	case 71: /*DeleteItemEx ItemDataEx*/
		{
			retint=LB_ERR;
			int nItems=GetCount();
			*oStr=0;
	...
	case 76: /*AddItemEx [iBefore],ItemDataEx;ItemText*/
		{
			if (!ItemDataArray) ItemDataArray = new GP_RarefiedArr<CString>(5,"");
	...
	case 77: /*SetItemTextEx [iItem],iItemDataEx;Text*/
		if (ItemDataArray)
		{
			char *tmppos=new char [(int)strlen(pos)+10];
			strcpy(tmppos,pos);
	...
	case 78: /*SetCurSelByItemDataEx ItemDataEx*/
		if (ItemDataArray)
		{
			retint=CB_ERR;
			int nItems=GetCount();
...
UINT GPDrLBox::GPSetProperty(char *iStr)
{
...
void GPDrLBox::ParentNotify(UINT message, LPARAM lParam)
{
	char txt[100];
...
int GPDrLBox::CharToItem(UINT nChar, UINT nIndex)
{
	return 0;
}

int GPDrLBox::CompareItem(LPCOMPAREITEMSTRUCT lpCompareItemStruct)
{
	return 0;
}

void GPDrLBox::DeleteItem(LPDELETEITEMSTRUCT lpDeleteItemStruct)
{
}

int GPDrLBox::VKeyToItem(UINT nKey, UINT nIndex)
{
	return -1;
}

HBRUSH GPDrLBox::OnCtlColor(CDC* pDC, CWnd* pWnd, UINT nCtlColor)
{
	HBRUSH hbr = NULL;
	...
	if (pWnd && pDC && pWnd->m_hWnd && pDC->m_hDC)
		GPUnit::ChildCtlColor(referGPU, pDC->m_hDC, pWnd->m_hWnd, hbr, CDragListBox::OnCtlColor(pDC, pWnd, nCtlColor), nCtlColor);

	return hbr;
}

char * GPDrLBox::GetSelectedItem(char *iStr)
{
	int Sel=GetCurSel();
	char tmp[200];
...
LRESULT GPDrLBox::WindowProc(UINT message, WPARAM wParam, LPARAM lParam)
{
	if (referGPU && referGPU[UnitId&0xFFFF])
	{
	...
	else lpr=CDragListBox::WindowProc(message, wParam, lParam);

	return lpr;
}

BOOL GPDrLBox::PreTranslateMessage(MSG* pMsg)
{
	if (referGPU && referGPU[UnitId&0xFFFF])
	{
...
void GPDrLBox::OnClose()
{
	if (referGPU!=NULL)	referGPU[UnitId&0xFFFF]->GPMSG->GPSendObjEvent(m_hWnd, UnitId,"OnClose","");
//	CDragListBox::OnClose();
}

void GPDrLBox::OnSizing(UINT fwSide, LPRECT pRect)
{
	BOOL EnableSz=TRUE;
	UINT SizStyle=referGPU[UnitId&0xFFFF]->SizStyle;
	...
	if (EnableSz) CDragListBox::OnSizing(fwSide, pRect);

	GetParent()->SendMessage(GPN_UPDATESIZE);

}

void GPDrLBox::OnDragMouseHover()
{
...
void GPDrLBox::OnMouseDragMove(PGPDRAGDROPSTRUCT pDrDrStruct, LPPOINT point)
{
...
void GPDrLBox::OnDragLButtonUp(PGPDRAGDROPSTRUCT pDrDrStruct, LPPOINT point)
{
	BOOL mDrop=referGPU[UnitId&0xFFFF]->m_DropNamesArray && referGPU[UnitId&0xFFFF]->m_DropNamesArray->GetIdByVal(pDrDrStruct->DragName)<0xFFFFFFFFL;
...
DWORD GPDrLBox::HighlightDropTarget(PGPDRAGDROPSTRUCT pDrDrStruct, CPoint point)
{
	BOOL mb_Out;
...
LRESULT GPDrLBox::OnNcHitTest(CPoint point)
{
	LRESULT ret=CDragListBox::OnNcHitTest(point);
	if (referGPU && referGPU[UnitId&0xFFFF])
...
void GPDrLBox::OnKillFocus(CWnd* pNewWnd)
{
...
void GPDrLBox::OnSetFocus(CWnd* pOldWnd)
{
	CDragListBox::OnSetFocus(pOldWnd);
	GPUNITINFO ui;
	GPUNITINFO uiold;
	if (referGPU && referGPU[UnitId&0xFFFF])
		referGPU[UnitId&0xFFFF]->GetUnitInfo(ui);
...
void GPDrLBox::OnSize(UINT nType, int cx, int cy)
{
	CDragListBox::OnSize(nType, cx, cy);
...
void GPDrLBox::OnHScroll(UINT nSBCode, UINT nPos, CScrollBar* pScrollBar)
{
	CDragListBox::OnHScroll(nSBCode, nPos, pScrollBar);
	if (referGPU && referGPU[UnitId&0xFFFF]->m_SizerArray)	Invalidate();
}

void GPDrLBox::OnVScroll(UINT nSBCode, UINT nPos, CScrollBar* pScrollBar)
{
	CDragListBox::OnVScroll(nSBCode, nPos, pScrollBar);
	if (referGPU && referGPU[UnitId&0xFFFF]->m_SizerArray)	Invalidate();
}

BOOL GPDrLBox::OnChildNotify(UINT message, WPARAM wParam, LPARAM lParam, LRESULT* pLResult)
{
	return CDragListBox::OnChildNotify(message, wParam, lParam, pLResult);
}

BOOL GPDrLBox::OnNotify(WPARAM wParam, LPARAM lParam, LRESULT* pResult)
{
	return CDragListBox::OnNotify(wParam, lParam, pResult);
}

int GPDrLBox::AddChar(char iChar, DWORD iFlags)
{
	int ret=0;
	if (iChar)
	{
...
int GPDrLBox::OnMouseActivate(CWnd* pDesktopWnd, UINT nHitTest, UINT message) 
{
	int ret=CDragListBox::OnMouseActivate(pDesktopWnd, nHitTest, message);
