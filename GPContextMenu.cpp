		...
		if (iItem->hSubMenu)
		{
			...
			if (iItem->hMenu)
			{
				...
				if (j<nItems)
				{
					...
					mii.fMask=MIIM_DATA;
					::GetMenuItemInfo(iItem->hMenu,j,MF_BYPOSITION,&mii);
					m_pGPMenu->UnRegisterMenuItem(mii.dwItemData);
...
DWORD CGPContextMenu::SetODCForMenu(CODContext *ipContext /*reserved*/, HMENU ihPopup)
{
	int pos;
	char *menuText=new char[2000];
	char *menuText2=menuText;
	if (ipContext && ihPopup)
		for(pos=::GetMenuItemCount(ihPopup)-1;pos>=0;pos--)
		{
			...
			*tmpPosD=0;
			mii.dwItemData=m_pGPMenu->RegisterMenuItem(((DWORD)ihPopup)|0x80000000,(UINT)mii.wID, ipContext->m_ContextName, menuText2, 1,ihPopup);
			tmpPos=menuText;
			while(*tmpPos)
			{
