...
void CTwain::GetIdentity()
{
	...
	m_AppId.Version.MajorNum = atoi(m_TermVersion+1); //Your app's version number
	m_AppId.Version.MinorNum = atoi(m_TermVersion+3);
	m_AppId.Version.Language = TWLG_USA;
	m_AppId.Version.Country = TWCY_USA;
	strcpy (m_AppId.Version.Info, m_TermVersion);
	m_AppId.ProtocolMajor = TWON_PROTOCOLMAJOR;
	m_AppId.ProtocolMinor = TWON_PROTOCOLMINOR;
	m_AppId.SupportedGroups = DG_IMAGE | DG_CONTROL;
	strcpy (m_AppId.Manufacturer, "Infotools");
	strcpy (m_AppId.ProductFamily, "BRIZ");
	strcpy (m_AppId.ProductName, "BRIZ");
