...
//			if ((int)strlen(astr[i])>0)
			{
				...
				else
				{
					char *pss=astr[i];
					UINT flg=0;
					if (strstr(CharSpec,"SIGN1")) flg|=0x0001;
					if (strstr(CharSpec,"SIGN2")) flg|=0x0002;
					if (strstr(CharSpec,"SIGN3")) flg|=0x0004;
					if (strstr(CharSpec,"SIGN4")) flg|=0x0008;
					if (strstr(CharSpec,"DIGIT")) flg|=0x0010;
					if (strstr(CharSpec,"LCENG")) flg|=0x0020;
					if (strstr(CharSpec,"UCENG")) flg|=0x0040;
					if (strstr(CharSpec,"LCRUS")) flg|=0x0080;
					if (strstr(CharSpec,"UCRUS")) flg|=0x0100;
					if (strstr(CharSpec,"HEX"))	  flg|=0x0200;

					while(*pss!=0 && CharSpec!=NULL)
					{
						if (flg!=0)
						{
							if (   ((flg&0x0001) && *pss>='!' && *pss<='/')
								|| ((flg&0x0002) && *pss>=':' && *pss<='@')
								|| ((flg&0x0004) && *pss>='[' && *pss<='`')
								|| ((flg&0x0008) && *pss>='{' && *pss<='~')
								|| ((flg&0x0010) && *pss>='0' && *pss<='9')
								|| ((flg&0x0020) && *pss>='a' && *pss<='z')
								|| ((flg&0x0040) && *pss>='A' && *pss<='Z')
								|| ((flg&0x0080) && *pss>='à' && *pss<='ÿ')
								|| ((flg&0x0100) && *pss>='À' && *pss<='ß')
								|| ((flg&0x0200) && ((*pss>='A' && *pss<='F')||(*pss>='a' && *pss<='f')))
								) {pss++;continue;}
						}
						else
						{
