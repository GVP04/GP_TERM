...
/*70*/	"FindImagesR","GetFileStat","GetMethods","RemoveFile","ResizeImage",
/*75*/	"","","","","",
/*80*/	"","","","","",
/*85*/	"","","","","",
/*90*/	"","","","","",
...
/*70*/	"[Attributes],[dtType C A W],[dtStart YYYYMMDDHHmmSS],[dtEnd],[ILName],[nImage];FileName","FileName","","[Attributes],[dtType C A W],[dtStart YYYYMMDDHHmmSS],[dtEnd YYYYMMDDHHmmSS];FileName","[Attributes],[dtType C A W],[dtStart YYYYMMDDHHmmSS],[dtEnd YYYYMMDDHHmmSS];SrcFile,DstFolder,Type,Size",
/*75*/	"","","","","",
/*80*/	"","","","","",
/*85*/	"","","","","",
/*90*/	"","","","","",
...
UINT CGP_TermDoc::DoFileSystemMethod(const char *iStr, char **oStr)
{
	...
	try
	{
		if (*iStr) 	
		{
			...
			case 7: /*SetVolumeLabel*/
				...
				break;
						...
						case 8: /*DecryptFile*/
							if (pos)
							{
								char *tmps=new char [(int)strlen(pos)+110];
								strcpy(tmps,pos);
								DWORD flag=0;
			...
			case 73: /*RemoveFile [Attributes],[dtType C A W],[dtStart YYYYMMDDHHmmSS],[dtEnd YYYYMMDDHHmmSS];FileName*/
			case 74: //ResizeImage [Attributes],[dtType C A W],[dtStart YYYYMMDDHHmmSS],[dtEnd YYYYMMDDHHmmSS];SrcFile,DstFolder,Type,Size		
				if (oStr || nfunc==73 || nfunc==74)
				{
					...
					if (possr++)
					{
						...
						case 73:
							{
								RemoveFile(possr, oStr, params, dwDateType, DtMin, DtMax);
							}
							break;
						case 74: //ResizeImage SrcFile,DstFile,Type,Size		
							{
								int iType=atoi(ExtractField(possr,3,","));
								int iSize=atoi(ExtractField(possr,4,","));
								CString csSrcFileName1=ExtractField(possr,1,",");
								CString csDstFileName1=ExtractField(possr,2,",");
								
								if (!params) params=0xFFFF;

								CFileFind ff;
								BOOL bFind=ff.FindFile(csSrcFileName1,0);
								UINT ret=0;
								...
								if (bFind)
								{
									...
									try
									{
										do
										{
											...
											dwMask=(params&dwMask);
											if ((((dwMask/2)|dwMask)&0x5555)==0x5555)
											{
												ff.GetCreationTime(CRTime);
												ff.GetLastWriteTime(LWTime);
												ff.GetLastAccessTime(LATime);
												if ((!(dwDateType&1) || (CRTime>=DtMin && CRTime<=DtMax)) &&
												   (!(dwDateType&2) || (LWTime>=DtMin && LWTime<=DtMax)) &&
												   (!(dwDateType&4) || (LATime>=DtMin && LATime<=DtMax)))
												{
													if (oStr && *oStr)
													{
														if (**oStr)  *(outpos++)=0x7F;
													...
													CString csSrcFileName=ff.GetFilePath();
													if (!ff.IsDirectory() && csSrcFileName.Find(csDstFileName1)<0)
													{
														CString csDstFileName=csDstFileName1+ff.GetFileName();
														WCHAR tmpWText[1000];
														MultiByteToWideChar(CP_ACP, 0,csSrcFileName , -1, tmpWText, 990);
														Image BitmapSrc(tmpWText);
														UINT thumbWidth=BitmapSrc.GetWidth();
														UINT thumbHeight=BitmapSrc.GetHeight();
														
														if (iType==0)
														{
															thumbHeight=thumbHeight*iSize/thumbWidth;
															thumbWidth=iSize;
														}
														else
														if (iType==1)
														{
															thumbWidth=thumbWidth*iSize/thumbHeight;
															thumbHeight=iSize;
														}
														else
														{
															thumbHeight=thumbHeight*iSize/100;
															thumbWidth=thumbWidth*iSize/100;;
														}
														
														Bitmap BitmapDst( thumbWidth,   thumbHeight, BitmapSrc.GetPixelFormat());
														{
															Graphics gr((Image *)&BitmapDst);
															gr.DrawImage(&BitmapSrc,0,0,thumbWidth,   thumbHeight);
														}

														CLSID pngClsid;
														CBitmapPlus::GetEncoderClsid(L"image/jpeg", &pngClsid);
														MultiByteToWideChar(CP_ACP, 0,csDstFileName , -1, tmpWText, 990);
														BitmapDst.Save(tmpWText,&pngClsid,NULL);
													}
